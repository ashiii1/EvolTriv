...
import "keen-slider/keen-slider.min.css";
import { ChevronLeft, ChevronRight } from "lucide-react";
import products from "@/data/products"; // <-- reuse your services data
...
const OurServices = () => {
  ...
  // Flatten all items from products.js
  const allServices = products.reduce((all, cat) => [...all, ...cat.items], []);
        ...
        <div className="relative">
          ...
          <div ref={sliderRef} className="keen-slider">
            {allServices.map((service, index) => (
              <div key={index} className="keen-slider__slide">
                <div className="bg-white dark:bg-[#00005c] rounded-lg shadow-md overflow-hidden h-[420px]">
                  <Link to={`/products/${slugify(service.name)}`}>
                    <img
                      src={service.image}
                      alt={service.name}
                      className="w-full h-64 object-cover"

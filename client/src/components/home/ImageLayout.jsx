...
const images = [
  ...
  }, // Top Large Image
  {
    ...
    text: [
      "Hands-On Learning: IoT, EVs & Beyond",
      "Practical Innovations for Engineers",
      "Empowering Students with IoT",
    ],
  }, // Bottom Left Image
  {
    slides: [
...
export default function ImageGrid() {
  ...
  useEffect(() => {
    ...
    const bottomInterval = setInterval(() => {
      setBottomIndex((prev) => (prev + 1) % images[2].slides.length);
    }, 4000);
  ...
  return (
    <div className="dark:bg-[#00005c] pb-12 bg-gradient-to-b from-[#118B50]/10 to-transparent">
      ...
      <div className="w-screen h-screen grid grid-cols-1 md:grid-cols-2 grid-rows-3 gap-4 p-4">
        {/* Top Large Image (2 columns wide on md+) */}
        ...
        {/* Bottom Left Static Image with Caption */}
        <div className="col-span-1 row-span-2 relative">
          <img
            src={images[1].src}
            alt="Bottom Left"
          ...
          <motion.p
            ...
            className="absolute inset-0 flex items-center justify-center text-white text-xl font-semibold bg-black/40"
          >
            {images[1].text[textIndex]}
          </motion.p>
        </div>

        {/* Bottom Right Scrolling Images */}
        {/* Bottom Right Scrolling Images */}
<div className="col-span-1 row-span-2 relative overflow-hidden h-[250px] sm:h-[300px] md:h-full">
  <motion.img
    key={bottomIndex}
    src={images[2].slides[bottomIndex]}
    alt="Bottom Right Image"
    className="w-full h-full object-cover rounded-lg"
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  ...
  />
          ...
          {/* Pagination Dots */}
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
            {images[2].slides.map((_, idx) => (
              <div
                key={idx}
                className={`w-3 h-3 rounded-full ${
                  idx === bottomIndex ? "bg-white" : "bg-gray-400"
                }`}
              ></div>
            ))}
          </div>

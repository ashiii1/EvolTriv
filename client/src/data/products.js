
const products = [
{
  "id": 1,
  "category": "Solar Energy Solutions",
  "items": [
    {
      "name": "Solar PV Consulting",
      "description": "Our Solar PV Consulting service delivers comprehensive, expert guidance to design, implement, and optimize solar photovoltaic systems for maximum performance and cost-efficiency. We start by conducting detailed site analysis and feasibility studies to understand your unique energy requirements and budget constraints. Our team creates customized system designs that incorporate advanced technology and best practices to ensure optimal panel placement and longevity. We provide in-depth financial planning and ROI forecasting, helping clients secure incentives and navigate regulatory requirements. Our approach emphasizes low-cost, fast, and high-quality service to deliver tangible results. By leveraging years of experience and cutting-edge tools, we ensure every project achieves sustainability goals and long-term reliability. Ultimately, our consulting transforms solar projects into efficient, profitable, and eco-friendly energy solutions.",
      "details": "Seven-line detailed description here...",
      "image": "/assets/Images/solarpv.jpg",
      "imageBeforeFeatures": "/assets/Images/b2.jpg",
      "imageBeforeSpecs": "/assets/Images/s1.jpg",
      "imageBeforeBenefits": "/assets/Images/s2.jpeg",
      "icon": "Sun",
     "features": [
        "Customized solar solutions tailored to site-specific and energy requirements",
        "Detailed feasibility studies with cost-benefit analysis and ROI projection",
        "Advanced system design for maximum efficiency and longevity",
        "Regulatory guidance and incentive planning to optimize project returns",
        "Accurate performance forecasting using advanced simulation tools",
        "Financial planning and funding recommendations for sustainable projects",
        "Low-cost and fast service delivery without compromising quality",
        "Sustainability-focused designs to reduce carbon footprint",
        "Expert guidance for residential, commercial, and industrial projects",
        "Full documentation including reports, schematics, and compliance certificates"
      ],
     "specifications": {
        "Service Type": "Consulting & Advisory",
        "Industries Covered": "Residential, Commercial, Industrial",
        "Project Size": "1kW - 100MW",
        "Design Tools Used": "PVsyst, Helioscope, AutoCAD, SAM (System Advisor Model)",
         },
     "benefits": [
        {
          "title": "Maximized Efficiency",
          "description": "Ensures optimal panel placement and system design for highest energy yield and sustained performance."
        },
        {
          "title": "Cost Reduction",
          "description": "Identifies the most efficient and affordable solutions to minimize upfront and long-term costs."
        },
        {
          "title": "Regulatory Compliance",
          "description": "Helps meet local, state, and national solar energy regulations seamlessly, avoiding delays and fines."
        },
       
        ]
    },
   {
  "name": "Solar Rooftop Installation",
  "description": "Our Solar Rooftop Installation service delivers high-quality solar panel systems for homes, commercial buildings, and industrial properties, combining efficiency, aesthetics, and long-term performance. We provide end-to-end service — from design and procurement to installation and post-installation support — ensuring a seamless experience. By integrating advanced technology and premium materials, we maximize energy yield and system longevity. Our approach emphasizes low-cost, best-quality, and fast service delivery to help clients achieve sustainable energy goals with minimal hassle.",
  "details": "Our Solar Rooftop Installation service provides efficient, eco-friendly energy generation directly on your property. We manage every step: site analysis, customized design, premium panel selection, expert installation, and reliable after-sales support. Using cutting-edge mounting systems, we ensure durability and optimal orientation for maximum energy output. Whether for residential, commercial, or industrial buildings, our installations lower electricity costs, promote sustainability, and deliver a quick return on investment. Our low-cost, best-quality, and fast service model guarantees excellence without delays or hidden costs.",
  "image": "/assets/Images/rooftopmain.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/s3.jpg",
  "imageBeforeBenefits": "/assets/Images/s4.jpg",
  "icon": "Sun",
  "features": [
    "Customized rooftop solar solutions tailored to property requirements",
    "Space-efficient solar panel installations for maximum yield",
    "On-grid and off-grid system configurations",
    "Quick and hassle-free installation process",
    "Premium monocrystalline/polycrystalline solar panels with long warranty",
    "Advanced mounting systems for durability and aesthetic appeal",
    "Remote monitoring and performance tracking",
    "Detailed site analysis and feasibility studies",
    "Low-cost, best-quality, fast service delivery",
    "Full documentation including warranties, compliance certificates, and performance reports"
  ],
  "specifications": {
    "System Size": "1kW - 500kW",
    "Panel Type": "Monocrystalline / Polycrystalline",
    "Warranty": "25 Years",
    "Installation Time": "3–7 Days (Depending on Project Size)",
   
  },
  "benefits": [
    { "title": "Cost Savings", "description": "Reduce electricity bills by up to 70% with rooftop solar energy, ensuring long-term financial benefits." },
    { "title": "Renewable Energy", "description": "Generate clean, green power directly on-site, reducing carbon footprint and promoting sustainability." },
    { "title": "Energy Independence", "description": "Gain greater control over your energy needs and reduce reliance on the grid." },
     ]
},
   
{
  "name": "Ground-Mounted Solar Installation",
  "description": "Our Ground-Mounted Solar Installation service provides robust, high-performance solar energy solutions for large-scale commercial, industrial, and utility projects. Combining durability, scalability, and advanced engineering, we deliver cost-effective energy systems that maximize sunlight utilization. Designed for open land installations, our approach offers flexibility, superior efficiency, and long-term reliability. From planning to execution, we ensure fast service without compromising quality, making renewable energy adoption easier and more sustainable.",
  "details": "Our Ground-Mounted Solar Installation service is ideal for projects where space, scalability, and high energy output are critical. We offer a full-service solution that includes site evaluation, customized design, equipment procurement, installation, and post-installation support. Our systems feature fixed mounts as well as advanced single-axis and dual-axis tracking mechanisms to optimize sunlight capture. We ensure compliance with industry standards and regulations, and our projects are engineered for durability, low maintenance, and long-term performance. Whether for industrial plants, solar farms, or utility-scale power projects, we deliver a solution that combines low cost, best quality, and fast service for maximum impact.",
  "image": "/assets/Images/groundmounted.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/s5.jpg",
  "imageBeforeBenefits": "/assets/Images/s6.jpg",
  "icon": "Sun",
  "features": [
    "High-capacity solar energy generation for large-scale projects",
    "Customizable tilt angles and tracking systems for maximum efficiency",
    "Fixed, single-axis, and dual-axis mounting options",
    "Robust mounting systems built to withstand harsh weather",
    "Space-optimized layouts for improved energy yield",
    "Comprehensive project management from concept to commissioning",
    "Use of premium materials for durability and long-term performance",
    "Advanced monitoring for system efficiency and reliability",
    "Low-cost, best-quality, and fast service delivery"
  ],
  "specifications": {
    "System Capacity": "100kW - 100MW",
    "Mounting Type": "Fixed / Single-axis / Dual-axis",
    "Lifespan": "25+ years",
  },
  "benefits": [
    {
      "title": "Maximum Sunlight Exposure",
      "description": "Advanced tilt and tracking systems ensure optimal sunlight capture throughout the day, significantly improving energy yield."
    },
    {
      "title": "Scalability",
      "description": "Designed to easily expand installations to meet growing energy demands."
    },
    {
      "title": "Low Maintenance",
      "description": "Engineered for long-term use with minimal upkeep, reducing operational costs."
    },
  
   
  ]
},

   {
  "name": "Solar Microgrid Construction",
  "description": "Our Solar Microgrid Construction service delivers reliable, self-sustaining power solutions tailored for communities, industries, and remote locations. Combining advanced solar technology with intelligent energy management, we design systems that ensure uninterrupted power supply while reducing dependency on traditional grids. Ideal for rural electrification, industrial facilities, and disaster-resilient energy needs, our microgrids integrate scalable solar arrays with advanced storage solutions to deliver sustainable, efficient, and resilient energy infrastructure.",
  "details": "Our Solar Microgrid Construction service offers complete planning, design, installation, and maintenance for off-grid and hybrid power systems. Each microgrid is custom-designed to match site-specific needs, integrating high-efficiency solar panels, lithium-ion or flow battery storage, and smart control systems. These microgrids are engineered for reliability and scalability, capable of providing continuous power even in remote or grid-challenged areas. We handle every stage of the project — from feasibility analysis and permitting to installation and commissioning — ensuring seamless delivery and long-term performance. Our service guarantees energy independence, cost savings, and reduced environmental impact.",
  "image": "/assets/Images/microgrid.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/s8.jpg",
  "imageBeforeBenefits": "/assets/Images/s9.jpg",
  "icon": "Sun",
  "features": [
    "Custom-designed off-grid and hybrid solar microgrids",
    "Integrated battery storage for uninterrupted power supply",
    "Smart control and monitoring systems for optimal performance",
    "Scalable designs tailored to community or industrial needs",
    "Rapid deployment with full end-to-end project execution",
    "High-quality materials for long-term reliability",
    "Advanced energy forecasting and load management",
    "Remote monitoring for proactive maintenance",
    "Environmentally sustainable design reducing carbon footprint"
  ],
  "specifications": {
    "Power Output": "10kW - 10MW",
    "Storage Type": "Lithium-ion / Flow Batteries",
    "Backup Time": "Up to 48 hours",
    "Installation Time": "6–16 Weeks (Depending on Project Size)"
  },
  "benefits": [
    {
      "title": "Energy Independence",
      "description": "Eliminates reliance on central grids, empowering communities and businesses with self-sustained energy."
    },
    {
      "title": "Reliable Power Supply",
      "description": "Ensures uninterrupted electricity even in remote locations or during grid failures."
    },
    {
      "title": "Eco-Friendly",
      "description": "Generates clean energy to drastically reduce greenhouse gas emissions and environmental impact."
    },
    
  ]
},

   {
  "name": "Solar Water Pump",
  "description": "Our Solar Water Pump solutions deliver efficient, eco-friendly, and cost-saving water pumping systems for agriculture, livestock, households, and industries. Operating entirely on solar energy, these pumps provide a reliable water supply in even the most remote locations without grid access. Designed for durability and performance, our systems reduce operating costs while promoting sustainability. We offer end-to-end service, including design, installation, and maintenance, ensuring smooth operation for years to come.",
  "details": "Our Solar Water Pump systems are engineered to provide sustainable and affordable water access for irrigation, drinking water, livestock, and industrial purposes. By eliminating reliance on electricity or diesel, they drastically cut costs and reduce environmental impact. Available in submersible and surface pump options, our solutions are tailored to your specific water needs. Using durable, corrosion-resistant materials and high-efficiency pump designs, these systems ensure consistent performance and long life. From feasibility assessment and custom design to installation and after-sales service, we deliver reliable water solutions that empower communities, farmers, and industries alike.",
  "image": "/assets/Images/waterpump.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/a2.jpg",
  "imageBeforeBenefits": "/assets/Images/a3.jpg",
  "icon": "Sun",
  "features": [
    "Fully solar-powered operation with zero fuel or electricity needs",
    "Available in both submersible and surface pump configurations",
    "High-efficiency pump designs tailored for irrigation, livestock, and household use",
    "Durable, corrosion-resistant construction for long-term reliability",
    "Customizable system design for specific site and water requirements",
    "Easy installation and low-maintenance operation",
    "Adaptable for farms, villages, industries, and remote locations",
    "Environmentally friendly technology reducing carbon emissions"
  ],
  "specifications": {
    "Power Range": "0.5HP - 10HP",
    "Pump Type": "Submersible / Surface",
    "Flow Rate": "Up to 60,000 liters per day (depending on configuration)",
  
  },
  "benefits": [
    {
      "title": "Zero Fuel Costs",
      "description": "Runs entirely on solar power, eliminating electricity and diesel expenses."
    },
    {
      "title": "Sustainable Water Supply",
      "description": "Provides reliable water access in off-grid and rural locations."
    },
    {
      "title": "Durable & Reliable",
      "description": "Built with robust, corrosion-resistant materials ensuring long-term performance."
    },
   
   
  ]
},
   {
  "name": "Solar Water Heater",
  "description": "Our Solar Water Heater systems provide energy-efficient, eco-friendly, and reliable hot water solutions for residential, commercial, and industrial applications. By utilizing solar energy, these systems drastically reduce electricity and gas consumption, offering long-term cost savings and environmental benefits. With premium stainless steel and copper tanks, advanced insulation, and weather-resistant design, our heaters deliver consistent performance in all climates while ensuring durability and low maintenance.",
  "details": "Our Solar Water Heater service covers end-to-end solutions, including system design, installation, and ongoing maintenance. We customize capacity and design based on your specific needs, whether for homes, hotels, hospitals, or industrial facilities. Featuring high heating efficiency of up to 95%, these systems ensure reliable hot water even during colder seasons or cloudy days. Built with robust stainless steel and copper tanks, they provide long-term durability and minimal upkeep. Integrated monitoring and smart controls allow efficient energy management, ensuring you save on costs while reducing your carbon footprint. With our commitment to low-cost, best-quality, and fast service, we provide a complete solution for sustainable water heating.",
  "image": "/assets/Images/waterheater.jpeg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/a4.jpg",
  "imageBeforeBenefits": "/assets/Images/a5.jpg",
  "icon": "Sun",
  "features": [
    "High-efficiency solar heating systems delivering up to 95% thermal performance",
    "Durable stainless steel and copper tank construction",
    "Works efficiently across all weather and seasonal conditions",
    "Custom capacity and design options for homes, hotels, hospitals, and industries",
    "Advanced insulation technology for maximum heat retention",
    "Energy monitoring and control systems for optimized performance",
    "Fast installation with minimal disruption",
    "Low-cost, best-quality, and fast service delivery"
  ],
  "specifications": {
    "Tank Capacity": "100L - 500L",
    "Heating Efficiency": "80-95%",
    "Material": "Stainless Steel / Copper",
    "Installation Time": "2–5 Days (Depending on Capacity)",
  },
  "benefits": [
    {
      "title": "Energy Savings",
      "description": "Cuts electricity and gas bills by up to 70% through efficient solar heating."
    },
    {
      "title": "Eco-Friendly Heating",
      "description": "Reduces carbon emissions by replacing fossil fuel–based heating with renewable energy."
    },
    {
      "title": "Reliable Hot Water Supply",
      "description": "Ensures consistent heating even during cloudy weather or colder climates."
    },
   
   
  ]
},
   {
  "name": "Solar PV Cleaning and Maintenance",
  "description": "Our Solar PV Cleaning and Maintenance service ensures maximum efficiency, reliability, and durability of your solar energy system. Dust, dirt, and environmental pollutants can reduce solar panel performance by up to 30%, but our professional cleaning and monitoring solutions restore and maintain optimal energy output. With advanced tools, preventive inspections, and performance tracking, we provide peace of mind and long-term cost savings.",
  "details": "We offer comprehensive cleaning and maintenance services tailored to residential, commercial, and industrial solar installations. Our process includes regular removal of dust, bird droppings, and debris using water jets and eco-friendly automated cleaning systems, ensuring panels stay spotless without damage. Beyond cleaning, we conduct preventative maintenance such as system health checks, performance monitoring, and fault detection. By scheduling quarterly or annual maintenance, clients maximize solar efficiency, avoid costly breakdowns, and extend system life. Our goal is to deliver low-cost, fast, and high-quality service so you can enjoy uninterrupted solar power for decades.",
  "image": "/assets/Images/solarpv.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/a6.jpg",
  "imageBeforeBenefits": "/assets/Images/a7.jpg",
  "icon": "Sun",
  "features": [
    "Regular dust, dirt, and debris removal for maximum solar efficiency",
    "Use of eco-friendly, non-abrasive cleaning solutions",
    "Performance monitoring and real-time fault detection",
    "Quarterly or annual maintenance schedules available",
    "Extends the operational lifespan of solar panels",
    "Comprehensive inspection of wiring, connections, and inverter performance",
    "Automated cleaning systems for large-scale installations",
    "Documentation and reports after each service visit",
    "Affordable pricing with flexible service plans",
    "Low-cost, best-quality, and fast service delivery"
  ],
  "specifications": {
    "Service Type": "Cleaning & Preventative Maintenance",
    "Frequency": "Quarterly / Annually (Custom Plans Available)",
    "Tools Used": "High-pressure water jets, automated robotic cleaners, eco-friendly agents",
  },
  "benefits": [
    {
      "title": "Maximized Energy Efficiency",
      "description": "Clean panels absorb maximum sunlight, boosting energy generation by up to 30%."
    },
    {
      "title": "Reduced Downtime",
      "description": "Proactive fault detection prevents unexpected breakdowns and costly interruptions."
    },
    {
      "title": "Long-Term Savings",
      "description": "Proper maintenance extends the lifespan of panels, reducing replacement and repair costs."
    },
    
  ]
}
  ]
},
  
  {
  "id": 2,
  "category": "Electric Mobility",
  "items": [
   {
  "name": "E-Bike Manufacturing",
  "description": "Our E-Bike Manufacturing service specializes in designing and producing high-performance electric bicycles that are perfect for both urban commuting and adventurous off-road riding. With a focus on sustainability, innovation, and rider comfort, we deliver cutting-edge e-bikes that combine power, style, and durability. Every model is engineered to provide maximum efficiency, smooth handling, and long-lasting reliability, making eco-friendly transportation accessible to everyone.",
  "details": "We offer end-to-end manufacturing of e-bikes, including frame design, motor integration, battery optimization, and smart technology features. Our bikes are crafted with lightweight yet durable materials to ensure comfort and strength. Customers can choose from various motor capacities, battery ranges, and design styles tailored to personal or commercial needs. With GPS integration, app connectivity, and safety-enhanced components, our e-bikes redefine the future of urban mobility. Our approach emphasizes low-cost, best-quality, and fast production, ensuring riders enjoy a sustainable, high-performance alternative to traditional transport.",
  "image": "/assets/Images/ebike.png",
  "imageBeforeFeatures": "/assets/Images/c2.jpg",
  "imageBeforeSpecs": "/assets/Images/c1.jpg",
  "imageBeforeBenefits": "/assets/Images/c2.jpg",
  "icon": "Zap",
  "features": [
    "Custom frame designs optimized for strength and lightweight handling",
    "Powerful brushless electric motors for smooth acceleration",
    "Long-lasting lithium-ion batteries with advanced battery management systems",
    "Fast-charging technology reducing downtime",
    "Smart connectivity with mobile apps, GPS, and Bluetooth integration",
    "Eco-friendly and sustainable manufacturing processes",
    "Ergonomic design for enhanced rider comfort",
    "Multiple riding modes: eco, sport, and turbo assist",
    "Durable components for urban and off-road use",
    "Low-cost, best-quality, and fast delivery cycles"
  ],
  "specifications": {
    "Motor Power": "250W - 1000W",
    "Battery Capacity": "36V - 52V",
    "Range Per Charge": "40-120 km",
    
    
  },
  "benefits": [
    {
      "title": "Sustainable Transportation",
      "description": "E-bikes reduce carbon footprint and fuel dependency, offering a greener mobility solution."
    },
    {
      "title": "High Performance & Comfort",
      "description": "Combines powerful motors, ergonomic designs, and durable frames for a superior riding experience."
    },
    {
      "title": "Smart Technology Integration",
      "description": "Advanced connectivity features like GPS tracking, Bluetooth, and mobile app integration enhance usability."
    },
   
  ]
},
   {
  "name": "E-Bike Repair",
  "description": "Our E-Bike Repair service provides comprehensive maintenance and repair solutions for all models and brands of electric bicycles. From minor tune-ups to major overhauls, we ensure your e-bike stays in top performance condition. Our expert technicians diagnose and fix issues quickly, using genuine parts and advanced tools. Whether you need battery replacement, motor servicing, or software updates, we deliver cost-effective, reliable, and fast repair solutions to get you back on the road without hassle.",
  "details": "We specialize in complete repair and maintenance services for electric bicycles, covering every component from the motor to the brakes. Our services include detailed diagnostics, battery testing and replacement, motor and controller repairs, tire and brake servicing, and advanced software troubleshooting. For added convenience, we offer both on-site repairs and remote support, ensuring riders never face unnecessary downtime. By providing quick turnaround times, affordable pricing, and high-quality workmanship, we help extend the lifespan of your e-bike and enhance its overall performance. Our low-cost, best-quality, and fast service approach makes us the go-to choice for e-bike riders.",
  "image": "/assets/Images/ebike2.jpg",
  "imageBeforeFeatures": "/assets/Images/c3.jpg",
  "imageBeforeSpecs": "/assets/Images/c3.jpg",
  "imageBeforeBenefits": "/assets/Images/ebb.png",
  "icon": "Zap",
  "features": [
    "Battery diagnostics, capacity testing, and replacement with high-quality cells",
    "Motor and controller repairs ensuring smooth acceleration and performance",
    "Brake inspection, adjustment, and replacement for maximum safety",
    "Tire replacement and puncture repair for reliable rides",
    "Software updates and troubleshooting to optimize system performance",
    "Electrical wiring inspection and correction of faults",
    "On-site and remote repair support options for customer convenience",
    "Preventive maintenance plans to avoid future breakdowns",
    "Affordable service packages tailored for regular riders and fleets",
    "Quick turnaround times with genuine spare parts"
  ],
  "specifications": {
    "Service Type": "Repair & Maintenance",
    "Coverage": "All E-Bike Brands",
    "Turnaround Time": "24-72 Hours",
   
  },
  "benefits": [
    {
      "title": "Extended Bike Lifespan",
      "description": "Regular maintenance and professional repairs ensure your e-bike performs efficiently for years."
    },
    {
      "title": "Quick and Reliable Service",
      "description": "Fast repair turnaround minimizes downtime so you can get back on the road quickly."
    },
    {
      "title": "Cost-Effective Maintenance",
      "description": "Affordable service packages reduce costs compared to frequent replacements."
    },
   
   
  ]
},
   {
  "name": "Convert Your Standard Bike into E-Bike",
  "description": "Our E-Bike Conversion service allows you to upgrade your existing standard bicycle into a powerful electric bike using advanced conversion kits. This cost-effective solution gives riders the benefits of an e-bike without purchasing a brand-new model. With options for different motor powers, battery capacities, and assist modes, you can customize the experience to match your lifestyle and terrain. Easy installation and compatibility with most bike types make it a seamless transition to electric mobility.",
  "details": "We specialize in transforming traditional bicycles into modern e-bikes with our high-quality conversion kits. The process includes integrating an electric motor, battery system, controller, and display for smooth and powerful rides. Our kits feature plug-and-play designs for easy installation and compatibility with mountain bikes, road bikes, and commuter bikes. Riders can enjoy customizable pedal-assist levels, extended range, and faster commutes while reducing fatigue. This eco-friendly upgrade not only reuses your existing bicycle but also promotes sustainable transportation at an affordable cost. With professional installation and after-sales support, we ensure your converted e-bike delivers comfort, reliability, and performance.",
  "image": "/assets/Images/ebike3.jpg",
  "imageBeforeFeatures": "/assets/Images/an1.jpg",
  "imageBeforeSpecs": "/assets/Images/an1.jpg",
  "imageBeforeBenefits": "/assets/Images/an2.jpg",
  "icon": "Zap",
  "features": [
    "Plug-and-play electric motor kits for easy installation",
    "Battery integration with long-range lithium power systems",
    "Customizable speed settings and multiple assist levels",
    "Lightweight design that doesn’t compromise bike handling",
    "Compatible with most standard bicycles including road, mountain, and commuter bikes",
    "Professional installation service available for precision fitting",
    "Durable and water-resistant components for all-weather riding",
    "LCD display for speed, distance, and battery monitoring",
    "Regenerative braking option in select kits",
    "Affordable upgrade packages with flexible configurations"
  ],
  "specifications": {
    "Motor Power": "250W - 750W",
    "Battery Capacity": "36V - 48V",
    "Range Per Charge": "40-80 km",
   
  },
  "benefits": [
    {
      "title": "Affordable E-Bike Alternative",
      "description": "Get the benefits of an electric bike at a fraction of the cost of buying new."
    },
    {
      "title": "Eco-Friendly Upgrade",
      "description": "Reuses your existing bicycle, minimizing waste and promoting sustainability."
    },
    {
      "title": "Customizable Experience",
      "description": "Choose motor power, battery size, and assist levels to match your riding style."
    },
   
   
  ]
}

  ]
}
,

{
  "id": 3,
  "category": "Student Support",
  "items": [
   {
  "name": "Workshops",
  "description": "Our Workshops provide interactive, expert-led training sessions that focus on practical knowledge and hands-on skills across various technical domains. Designed for students and early professionals, these workshops bridge the gap between theoretical learning and industry applications, ensuring participants gain confidence and job-ready skills.",
  "details": "We organize specialized workshops that combine theory with live demonstrations and practical activities. Covering topics such as renewable energy, electric mobility, IoT, AI, robotics, and more, our workshops are tailored to meet industry standards. Sessions are led by experienced trainers and industry professionals, ensuring participants gain real-world exposure. Each workshop includes project-based activities, Q&A sessions, and networking opportunities to enhance overall learning and confidence. On successful completion, participants receive certification to showcase their achievements.",
  "image": "/assets/Images/stu1.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/srrumain.jpg",
  "imageBeforeBenefits": "/assets/Images/d1.jpg",
  "icon": "Battery",
  "features": [
    "Expert-led sessions with real industry professionals",
    "Live demonstrations and hands-on project activities",
    "Workshops on trending technologies (AI, IoT, EVs, Renewable Energy, etc.)",
    "Flexible schedules with both online and offline options",
    "Interactive Q&A sessions for doubt clearing",
    "Networking with peers and mentors",
    "Project-based learning with practical exposure",
    "Certification upon successful completion"
  ],
  "specifications": {
    "Duration": "1-4 Weeks",
    "Mode": "Online / Offline",
    "Certification": "Provided upon Completion",
    "Batch Size": "20-50 Participants"
  },
  "benefits": [
    {
      "title": "Enhanced Learning",
      "description": "Practical, hands-on sessions strengthen technical understanding and confidence."
    },
    {
      "title": "Industry Exposure",
      "description": "Workshops are aligned with real-world applications and current industry practices."
    },
    {
      "title": "Networking Opportunities",
      "description": "Directly connect with industry experts, mentors, and like-minded peers."
    },
   
  ]
},
{
  "name": "Internships",
  "description": "Our Internship programs provide students and fresh graduates with the opportunity to work on real-world projects, guided by industry professionals. These internships help bridge the academic-to-industry gap by offering practical knowledge, skill enhancement, and valuable work experience.",
  "details": "We offer structured internship programs that allow participants to gain hands-on experience across different domains such as renewable energy, e-mobility, software development, IoT, and more. Interns are assigned to real projects where they collaborate with professionals and learn industry best practices. Each internship includes mentorship sessions, progress tracking, and career guidance. With hybrid options, students can gain exposure both online and on-site. Selected candidates may also receive stipends based on performance and project contributions.",
  "image": "/assets/Images/stu2.jpg",
  "imageBeforeFeatures": "/assets/Images/b1.jpg",
  "imageBeforeSpecs": "/assets/Images/st1.jpg",
  "imageBeforeBenefits": "/assets/Images/st2.jpg",
  "icon": "Battery",
  "features": [
    "Opportunity to work on live, real-world projects",
    "Mentorship and guidance from experienced professionals",
    "Skill enhancement in technical and soft skills",
    "Exposure to industry tools, processes, and workflows",
    "Hybrid mode for flexible participation (online + offline)",
    "Evaluation and feedback sessions to track progress",
    "Possibility of stipends for selected candidates",
    "Certificate of completion to validate internship experience"
  ],
  "specifications": {
    "Duration": "1-6 Months",
    "Mode": "Hybrid (Online & Offline)",
    "Stipend": "Available for Selected Candidates",
  },
  "benefits": [
    {
      "title": "Career Growth",
      "description": "Internships strengthen resumes and prepare students for job opportunities."
    },
    {
      "title": "Practical Knowledge",
      "description": "Apply theoretical concepts in real-world scenarios to gain true expertise."
    },
    {
      "title": "Skill Development",
      "description": "Enhance problem-solving, teamwork, and communication skills crucial for career success."
    },
   
  ]
}

  ]
}
]

export default products;

...
const ProductDetailDialog = ({ product }) => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl  max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">{product.name}</DialogTitle>
          <DialogDescription>Discover the features and benefits of our {product.name}</DialogDescription>
        </DialogHeader>
  ...
  {product.imageBeforeFeatures && (
    <img src={product.imageBeforeFeatures} alt="Before Features" className="w-full h-48 object-cover rounded-lg" />
  )}
  ...
  {/* Image Before Technical Specifications */}
  {product.imageBeforeSpecs && (
    <img src={product.imageBeforeSpecs} alt="Before Specifications" className="w-full h-48 object-cover rounded-lg" />
  )}
  ...
  {product.imageBeforeBenefits && (
    <img src={product.imageBeforeBenefits} alt="Before Benefits" className="w-full h-48 object-cover rounded-lg" />
  )}
  ...
  <div className="space-y-4">
    ...
    <div className="grid gap-4">
      {product.benefits.map((benefit, index) => (
        ...
        <div className="flex justify-between pt-4">
          <button onClick={handleDownloadBrochure} className="flex items-center dark:text-white gap-2 text-sm text-gray-600 dark:hover:text-blue-600 hover:text-[#118B50]">
            <Download className="w-4 h-4 dark:text-white" />
            Download Brochure
          </button>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Link to the dedicated service page */}
            <Link to={`/products/${slugify(product.name)}`} className="text-sm text-[#118B50] hover:underline">
              Open full page →
            </Link>
          </div>
        </div>
  ...
  return (
    <Link to={`/products/${slug}`}>
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -20 }}
        className="bg-white rounded-xl shadow-lg overflow-hidden group dark:bg-black cursor-pointer hover:shadow-2xl transition-shadow"
      >
        <div className="relative h-64 overflow-hidden">
          <img
            src={product.image}
...
const ProductsPage = () => {
  ...
  const getAllProducts = () => products.reduce((allItems, category) => [...allItems, ...category.items], []);

  const getCurrentProducts = () => {
    if (selectedCategory === "All") return getAllProducts();
    return products.find((cat) => cat.category === selectedCategory)?.items || [];
  };
    ...
    <div className="min-h-screen bg-[#C1D8C3] w-full overflow-hidden">
      <div className="max-w-7xl mx-auto px-4  pt-32  py-16 w-full overflow-hidden dark:bg-[#0B0B45]">
        <Tabs defaultValue={selectedCategory} className="space-y-8">
          ...
          {products.map((category) => (
            <TabsContent key={category.id} value={category.category}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {category.items.map((product) => <ProductCard key={product.name} product={product} />)}
              </div>
            </TabsContent>
          ))}
        </Tabs>
      ...
      <div className="bg-[#C1D8C3] dark:bg-[#00005c] py-16">
        <div className="max-w-7xl mx-auto px-4 text-center dark:text-black">
          ...
          <Link to={"/contact"}>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-[#118B50] dark:bg-white dark:text-black text-white px-8 py-3 rounded-lg font-medium hover:bg-[#008075] transition-colors">
              Contact Us
            </motion.button>
          </Link>
        </div>
